...
while line:
    ...
    if (words[0]=="element" and words[1]=="vertex"):
        ...
        ply_out.write("property float nx\n")
        ply_out.write("property float ny\n")
        ply_out.write("property float nz\n")
    ...
    if (inBody == True):
        ply_out.write(str(round((float(words[0])-mid_x),2)) + " " + str(round((float(words[2]) - min_z),2)) + " " + str(-1 * round((float(words[1])-mid_y),2)) + " " + str(-1 * float(words[3])) + " " + str(words[5]) + " " + str(words[4]) + "\n")
